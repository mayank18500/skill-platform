                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      <div className="flex items-center space-x-1 text-sm text-gray-500">
                        ...
                        <span>{new Date(swap.created_at).toLocaleDateString()}</span> {/* FIXED: used created_at */}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-4 mb-3">
                      <div className="flex items-center space-x-2">
                        <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center">
                          <span className="text-white text-xs font-bold">
                            {getUserName(swap.from_user_id).charAt(0)} {/* FIXED: used from_user_id */}
                          </span>
                        </div>
                        <span className="font-medium text-gray-900">
                          {getUserName(swap.from_user_id)} {/* FIXED: used from_user_id */}
                        </span>
                      </div>
                      
                      <div className="flex items-center space-x-2">
                        <span className="px-2 py-1 bg-blue-100 text-blue-700 text-sm rounded-full">
                          {swap.skill_offered} {/* FIXED: used skill_offered */}
                        </span>
                        <span className="text-gray-400">↔</span>
                        <span className="px-2 py-1 bg-purple-100 text-purple-700 text-sm rounded-full">
                          {swap.skill_wanted} {/* FIXED: used skill_wanted */}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="w-8 h-8 bg-gradient-to-r from-green-400 to-blue-500 rounded-full flex items-center justify-center">
                          <span className="text-white text-xs font-bold">
                            {getUserName(swap.to_user_id).charAt(0)} {/* FIXED: used to_user_id */}
                          </span>
                        </div>
                        <span className="font-medium text-gray-900">
                          {getUserName(swap.to_user_id)} {/* FIXED: used to_user_id */}
                        </span>
                      </div>
                    </div>
                    ...
                    {selectedSwap === swap.id && (
                      <div className="mt-4 space-y-2 text-sm text-gray-600">
                        ...
                        <div><strong>Created:</strong> {new Date(swap.created_at).toLocaleString()}</div> {/* FIXED: used created_at */}
                        <div><strong>Updated:</strong> {new Date(swap.updated_at).toLocaleString()}</div> {/* FIXED: used updated_at */}
                        {swap.feedback && (
                          <div>
